...
def LinearQuantizeOut(x, bit):
    ...
    delta = torch.max(x) - torch.min(x)
    y = x.clone()
    if delta > 0: # tensor x is not a constant
        stepSizeRatio = 2.**(-bit)
...
# SFQ random offset
def LinearQuantizeOutWithOffset(x, bit, mean, std):
    minQ = torch.min(x)
    delta = torch.max(x) - torch.min(x)
    y = x.clone()
    if delta > 0:  # tensor x is not a constant
        ...
        stepSize = stepSizeRatio * delta.item()
        # 生成偏移量
        offset = torch.normal(mean=torch.full_like(x, fill_value=mean), std=torch.full_like(x, fill_value=std))
        offset = torch.round(offset)  # 四舍五入到最近的整数
        # 计算量化索引，考虑偏移量
        index = torch.clamp(torch.floor((x - minQ.item()) / stepSize) + offset, 0, (2.**(bit) - 1))
        y = index * stepSize + minQ.item()
