...
def wage_init_(tensor, bits_W, factor=2.0, mode="fan_in"):
    ...
    if dimensions < 2: raise ValueError("tensor at least is 2d")
        ...
        num_input_fmaps = tensor.size(1) # num of input channels
        receptive_field_size = 1
        if tensor.dim() > 2: # torch.Size([6, 1, 5, 5])
            receptive_field_size = tensor[0][0].numel() # num of kernel elements
        fan_in = num_input_fmaps * receptive_field_size # num of input elements
